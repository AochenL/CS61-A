...
def merge(incr_a, incr_b):
    ...
    # one of the sequences has no element
    while next_a == None and next_b:
        yield next_b
        next_b = next(iter_b, None)
    while next_b == None and next_a:
        yield next_a
        next_a = next(iter_a, None)
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    check = type(withdraw(0, old_pass))
    if check is str:
        return 'Incorrect password'

    def joint_withdraw(amount, pw):
        nonlocal old_pass, new_pass
        return withdraw(amount, old_pass if pw == new_pass else pw)
