...
def is_bst(t):
    ...
    def bst_min(t):
        ...
        if len(branches) == 1:
            return min(t.label, branches[0].label)
        else:
            return bst_min(branches[0])
    ...
    def bst_max(t):
        ...
        branches = [b for b in t.branches]
        if len(branches) == 1:
            return max(t.label, branches[0].label)
        else:
            return bst_min(branches[0])
    ...
    if len(branches) > 2:
        ...
        b = branches[0]
        if b.label <= t.label:
            return is_bst(b) and bst_min(b) <= t.label 
        else:
            return is_bst(b) and bst_max(b) > t.label
    else:
        left = branches[0]
        right = branches[1]
...
def store_digits(n):
    ...
    if n < 10:
        return Link(n)
    s = Link(n // 10, Link(n % 10))
    while s.first > 10:
        tmp = Link(s.first // 10, s)
        s.first %= 10
        s = tmp
    return s
...
def deep_map(f, link):
    ...
    if not isinstance(link.first, Link):
        k.first = f(link.first)
    else:
        k.first = deep_map(f, link.first)

    k.rest = deep_map(f, link.rest)
    return k
