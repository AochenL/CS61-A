...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    # if m is a planet, it is balanced
    if is_planet(m):
        return True
    # check if the torque applied by its left arm 
    # is equal to that applied by its right arm
    ...
    if right_length * right_weight != left_length * left_weight:
        return False
    # check if each of the mobiles hanging at the end of its arms
...
def quadratic(x, a, b, c):
    ...
    v1, v2 = f(lower), f(upper)
    extreme_p = -b/(2*a)
    if extreme_p > lower and extreme_p < upper:
        extreme_v = f(extreme_p)
    else:
        extreme_v = v1
    lower_v, upper_v = min(v1, v2, extreme_v), max(v1, v2, extreme_v)
    return interval(lower_v, upper_v)
...
def check_par():
    ...
    True
    """
    r1 = interval(5, 6) # Replace this line!
    r2 = interval(8, 9) # Replace this line!
    return r1, r2
