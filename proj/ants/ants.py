...
class Ant(Insect):
    ...
    def add_to(self, place):
        ...
        #print("Note: insect " + str(self.food_cost) + "added to " + self.place.name)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        search_range = 0
        while search_place != beehive:
            if search_place.bees and self.min_range <= search_range <= self.max_range:
                return rANTdom_else_none(search_place.bees)
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        for b in bees[:]:
            ...
            if self.armor <= 0 and b.place: # if fireAnt dies and if the bee is not removed from its place
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        # BEGIN Problem 10
        "*** YOUR CODE HERE ***"
        # action of the ant it contains 
        ant = self.contained_ant
        if ant: 
            ant.action(gamestate)
        # action of TankAnt itself
        bees = self.place.bees
        if bees:
            for b in bees[:]:
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"        
        # imposter queens reduce their own armor to 0
        if self.imposter:
            Ant.reduce_armor(self, self.armor)
            return
        # true queen """Throw a leaf at the nearest Bee in range."""
        self.throw_at(self.nearest_bee(gamestate.beehive))
        # true queen doubles the damage of ants behind her
        ...
        while place:
            ...
            if place.ant and isinstance(place.ant, ContainerAnt) and place.ant.contained_ant and (not place.ant.contained_ant.buffed):
                ant = place.ant.contained_ant
...
def apply_status(status, bee, length):
    ...
    def alt_status(gamestate):
        ...
        if length > 0:
            ...
            length -= 1
        elif length == 0 and status == make_scare:
            bee.is_scared = True
            bee.scare = False
