...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            edits = code_char
            res, cost = helper_align(skeleton_idx + 1, code_idx + 1) 
            edits += res
            possibilities.append((edits, cost))
        # Insert
        edits = "+[{0}]".format(code_char)
        res, cost = helper_align(skeleton_idx, code_idx + 1)
        cost += 1
        edits += res
        possibilities.append((edits, cost))
        # Delete
        edits = "-[{0}]".format(skel_char)
        res, cost = helper_align(skeleton_idx + 1, code_idx)
        cost += 1
        edits += res
        possibilities.append((edits, cost))
...
def num_splits(s, d):
    ...
    """
    "*** YOUR CODE HERE ***"
    # a number in the list can either be partitioned to the first group or the second group
    total = 0
