...
def copycat(lst1, lst2):
    ...
    ['c', 'c', 'c']
    """
    def copycat_helper(lst1, lst2, index, lst):
        if index >= len(lst1) or index >= len(lst2):
            return lst
        if lst2[index] >= 0:
            lst += [lst1[index]] * lst2[index]
        else:
            lst = lst[:lst2[index] - 1]
        return copycat_helper(lst1, lst2, index + 1, lst)
    return copycat_helper(lst1, lst2, 0, [])
